...
class Game:
    ...
    def get_shots_for_game(self):
        """
        Parses through Game object's json_file and returns a data frame containing 
        all shots taken in that game with several features related to the shots.
        ...
        feature_list = ["shot id", "team_id", "team_name", "player_id", "player_name", "play pattern", 
                        "x start location", "y start location", "duration", "outcome", "technique", 
                        "first time", "x gk position", "y gk position", "type of shot",
                        "num opponents within 5 yards", "num opponents between shot and goal", "statsbomb xg"]
        ...
        for events in self.json_file:
            ...
            if events['type']['name'] == 'Shot':
                ...
                if "freeze_frame" in events["shot"]:
                    ...
                    for player in events["shot"]["freeze_frame"]:
                        ...
                        if (player['teammate'] == False) and (player['position']['name'] != 'Goalkeeper'):
                            if self.check_player_btwn_shot_and_goal(x_start_location, y_start_location, x_player, y_player):
                ...
                features.append([shot_id, team_id, team_name, player_id ,player_name, play_pattern, 
                                 x_start_location, y_start_location, duration, outcome, technique, 
                                 first_time, x_gk_pos, y_gk_pos, type_shot, num_opponents_5_yards,
                                 num_opponents_between_goal, xg])
        
        #create data frame
        shot_df = pd.DataFrame(features, columns=feature_list).set_index("shot id")

        self.shot_df = shot_df
        
        return shot_df
    
    def check_player_btwn_shot_and_goal(self, x_shot, y_shot, x_player, y_player):
        """
        Helper function for get_shots_for_game(). 
        Checks whether a player in a statsbomb freeze frame is between the shot location, and the two lines
    ...
    def get_events_for_game(self):
        """
        Parses through Game object's json_file and returns a data frame containing 
        all all shots, passes, ball receipts and carries performed in that game 
        with several features related to those events.
            - eventid
            - x start location
            - y start location
        ...
        -------
        pandas.DataFrame
            - data frame containing all shots, passes, ball receipts and carries
            performed in the specified game with several features related to those events
        """

        feature_list = ["event id", "event name", "team_id", "team_name", "player_id", "player_name",
        ...
        for events in self.json_file:
            if events['type']['name'] in ["Pass", "Ball Receipt*", "Carry", "Shot"]:
                ...
                features.append([event_id, event_name, team_id, team_name, player_id, player_name, 
                                 x_start, y_start, x_end, y_end, xg, related])   
        
        events_df = pd.DataFrame(features, columns = feature_list).set_index("event id")
...
def fetch_matches_for_season(competition_id, season_id, verbose=True):
    ...
    """
    req = requests.get("https://raw.githubusercontent.com/statsbomb/open-data/master/data/matches" +
                       f"/{competition_id}" +
                       f"/{season_id}.json"
                      ).text
    ...
    game_num_dict = {}
    if verbose:
        print(f"\nFetching matches for season_id {season_id} of competition_id {competition_id}...")
    for i, game_num in enumerate(game_nums):
        game_num_dict[game_num] = Game(requests.get(base_url_string + str(game_num) + ".json").text)
        if verbose:
            sys.stdout.write('\r')
            sys.stdout.write(f"[%-{len(game_nums)-1}s] %d%%" % ('='*i, (100/(len(game_nums)-1))*i))
            sys.stdout.flush()
...
def fetch_seasons_for_league(competition_id, verbose=True):
    ...
    for season_name, season_id in all_seasons_id.items():
        season = fetch_matches_for_season(competition_id, season_id, verbose=verbose)
...
def get_shots_for_league(league_dict):
    ...
    league_dict : dict of dicts 
        - maps season id's to inner dictionaries, which themselves map 'Game' object with a 
        json_file attribute (the event data for that game as a JSON file).
...
def draw_pitch(axis, rotate=False):
    ...
    if not rotate:
        for lines in line_coords:               
            axis.plot(lines[0], lines[1], color = 'grey', linewidth = line_width, alpha = alpha)
        ...
        axis.plot(x1, x2, color = 'grey', linewidth = line_width, alpha = alpha)
        axis.plot(x3, x4, color = 'grey', linewidth = line_width, alpha = alpha)
        axis.plot(x5, x6, color = 'grey', linewidth = line_width, alpha = alpha)
        
    else:
        for lines in line_coords:               
            axis.plot([-(lines[1][0]-40) + 80, -(lines[1][1]-40) + 80], [lines[0][0], lines[0][1]], color = 'grey', 
...
def plot_shot_freeze_frame(game, shot_id, axis):
    ...
    game : Game
        - game object 
    shot_id : str
        - shot_id of shot to plot. Must be a valid index in game object's shot data frame
    axis : matplotlib.axes._subplots.AxesSubplot
        - matplotlib axis object on which to plot shot freeze frame
    ...
    None
    """
    assert "shot_df" in dir(game), "Game object must have a shot data frame. Call self.get_shots_for_game()"
    assert shot_id in game.shot_df.index, "Cannot find specified shot_id in this game's shot data frame"
